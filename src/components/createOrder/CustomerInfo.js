    ...
    try {
      ...
      if (customerId && cart.length) {
        ...
        const orderResult = await dispatch(
          createNewOrder(customerId, note)
        );

        const orderId = orderResult.data._id;

        if (orderId) {
          const orderDetailPromises = cart.map(
            async (orderDetail) => {
              await dispatch(
                createNewOrderDetail(orderId, orderDetail)
              );
            }
          );
          ...
          enqueueSnackbar(
            `Create successfully Order: ${orderResult.data.orderCode}`,
            { variant: 'success' }
          );
        }
      }
